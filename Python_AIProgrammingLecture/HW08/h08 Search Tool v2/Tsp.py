...
class Tsp(Problem):
    ...
    def execute(self):
        ...
        p = self.createProblem()    # 'p': (numCities, locations, table)
        # Call the search algorithm
        solution, self.result = self.steepestAscent(p)
    ...
    def steepestAscent(self, p):
        current = self.randomInit(p)   # 'current' is a list of city ids
    ...
    def mutants(self, current, p): # Inversion only
        n = p[0]
        neighbors = []
        count = 0
        triedPairs = []
